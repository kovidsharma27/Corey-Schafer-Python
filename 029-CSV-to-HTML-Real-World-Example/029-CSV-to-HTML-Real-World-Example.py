# This is a real world example usage of csv module of python.
# We got a patron csv file and wanted to make a html result output that contains contributers names.
# In this patron csv file there are people who want to show up in contributers page and people who don't want to.
# So we will be making a html result output with only names of people who want to show up in contributers page. 
...
with open('patrons.csv','r') as data_file:
    csv_data = csv.DictReader(data_file) # First row (headers) auto-consumed by DictReader

    next(csv_data) # Skip the "1 + Reward" description row (the reward tier explanation, not a real patron)

    for line in csv_data:
        if line['FirstName'] == 'No Reward':
            break # Stop collecting names — only "1+ Reward" tier patrons opted into the contributors page
